<template>
  ...
  >
    <div class="p-4 flex justify-between items-center border-b">
      ...
      <button @click="fecharSacola" class="text-gray-500 hover:text-black">
        ✕
      </button>
    </div>

    <div class="p-4">
      <!-- Conteúdo futuro da sacola -->
...
function enviarPedidoParaWhatsApp() {
  const numeroWhatsApp = '5592988313548' // Altere para o número de atendimento da sua loja

  let mensagem = '*Olá! Gostaria de fazer um pedido:*\n\n'

  sacola.itens.forEach((item, index) => {
  ...
  const total = sacola.itens.reduce((sum, item) => sum + item.preco * item.quantidadeSelecionada, 0)
